...
public class ProclipsingSkyscraperOptimizationCopy extends PApplet{
	...
	ColumnGridLine[] NSGridLine;
	ColumnGridLine[] EWGridLine;
	...
	int[] NSdIndex, EWdIndex;
	int minNSRange,maxNSRange,minEWRange,maxEWRange;
		...
		// DropdownList is of type ControlGroup.
		  ...
		  if (theEvent.isGroup()) {
		    // check if the Event was triggered from a ControlGroup
			  ...
			  NSGridLine = new ColumnGridLine[5];
			  EWGridLine = new ColumnGridLine[5];
			  NSdIndex = new int[5];
			  EWdIndex = new int[5];
			  for (int x=0; x<5; x++){
				  NSGridLine[x] = (ColumnGridLine) mySkyscraper.myColumnGrid.myNSLines.get(x);
				  NSdIndex[x] = NSGridLine[x].dist;
			  }
			  
			  for (int x=0; x<5; x++){
				  EWGridLine[x] = (ColumnGridLine) mySkyscraper.myColumnGrid.myEWLines.get(x);
				  EWdIndex[x] = EWGridLine[x].dist;
			  }
			  int NSbefore = glIndex[0]-1;
			  int NSafter = glIndex[0]+1;
			  int EWbefore = glIndex[1]-1;
			  int EWafter = glIndex[1]+1;
			  
			  
			  
			  if (glIndex[0]!=0 || glIndex[1]!=0){
				  minNSRange = NSdIndex[NSbefore]+1;
				  maxNSRange = NSdIndex[NSafter]-1;
				  minEWRange = EWdIndex[EWbefore]+1;
				  maxEWRange = EWdIndex[EWafter]-1;
				  
			  } else {
				  minNSRange = 0;
				  maxNSRange = NSdIndex[glIndex[0]+1]-1;
				  minEWRange = 0;
				  maxEWRange = NSdIndex[glIndex[0]+1]-1;
				  for (int x=0; x<5; x++){
					  System.out.print(NSbefore);
					  System.out.println(" "+NSdIndex[x]);
					  System.out.print(EWafter);
					  System.out.println(" "+EWdIndex[x]);
				  }
			  ...
			  if (ddlName == " Move NS GridLine") {
				  ...
				  sl2.setValue(NSdist);
				  sl2.setRange(minNSRange, maxNSRange);
				  sl2.setVisible(true);
			  } else if (ddlName == " Move EW GridLine"){
				  sl2.setVisible(false);
				  sl1.setValue(EWdist);
				  sl1.setRange(minEWRange, maxEWRange);
				  sl1.setVisible(true);
			  }
